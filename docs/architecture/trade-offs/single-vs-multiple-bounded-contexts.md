...
This document was written while ChatApp was still a single messaging context, and it
predicted that the split should wait for real friction. The split did come, across three
iterations: authentication, then authorization, then billing.

The trigger was the one named above — word overloading. "User" stopped meaning one thing.
In messaging it is a message sender; in Identity it is an account with credentials and
external logins; in Billing it is a subscriber with a plan. Trying to serve all three from
one `AppUser` was what made the boundary visible.

The layout that shipped is flatter than the one sketched above. Rather than nesting each
context in its own folder, the projects sit side by side under `backend/src/` and carry the
context in the project name:

```
Chat.Domain / Chat.Application / Chat.Infrastructure          ← messaging
Chat.Identity.Domain / .Application / .Infrastructure         ← identity
Chat.Billing.Domain / .Application / .Infrastructure          ← billing
Chat.Api                                                      ← composes all three
```

The nested version reads better in a diagram, but the flat one matches how the .NET
tooling actually behaves: solution filters, project references, and `dotnet new` all key
off project names, so the folder nesting bought nothing the naming did not already give.

The prediction that held up best was the sharing rule. The contexts share only `Guid UserId`
and no domain types, and that is what let billing land without touching messaging or auth.
